...
<style>
.login-body {
  background-image: url("../assets/images/nen/Nhan-vien-pha-che.jpg");
...
<body >
    ...
    {  
        ...
        FROM taikhoannv JOIN nhanvien on nhanvien.IdNV = taikhoannv.IdNV WHERE Email  = '$tk' AND password = '$mk' AND nhanvien.IdChuVu = '3'";
        $result = mysqli_query($conn, $queryDM);
        if((mysqli_num_rows($result)!= 0))
        {
            while($row = mysqli_fetch_array($result))
            {
                $_SESSION['id_pc'] = $row["IdNV"];
    ...
    <div class="login-body">
        <div class="login">
            <h3 class="login-title">Đăng nhập pha chế</h3>
